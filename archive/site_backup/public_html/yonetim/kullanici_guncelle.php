...
include("baglanti/sayfa_yetki_fonksiyonu.php");
 ...
 sayfayetkikontrol(7); 
  ...
  if(isset($_POST['kullaniciguncelle']) && $_POST['kullaniciguncelle']) {                               
  
    $sicilno = isset($_POST["sicilno"]) ? addslashes($_POST["sicilno"]) : '';
    $adi = isset($_POST["adi"]) ? addslashes($_POST["adi"]) : '';
    $kullaniciadi = isset($_POST["kullaniciadi"]) ? addslashes($_POST["kullaniciadi"]) : '';
   if (isset($_POST["sifre"]) && $_POST["sifre"]!='') { $sifre = addslashes(md5($_POST["sifre"]));}
    $mail = isset($_POST["mail"]) ? addslashes($_POST["mail"]) : '';                       
    $yoneticidurumu = isset($_POST["yoneticidurumu"]) ? temizlikimandan($_POST["yoneticidurumu"]) : ''; 
    
    $kayityapan =temizlikimandan($_SESSION['kullaniciid']);
    $bugununtarihi=date('Y.m.d H:i:s');   
    $durum='1'; 
    $idalma=isset($_POST["guncelkullaniciid"]) ? temizlikimandan($_POST["guncelkullaniciid"]) : '';   
