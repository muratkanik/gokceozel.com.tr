...
$allowed_cols = array('et.id', 'eu.tr_isim', 'et.tr_baslik', 'dk.isim', 'et.tr_seo_score', 'et.son_guncelleme');
...
// Try Catch Block for Query
try {
    $baglanti = sorgu($sorgu, $baglan);
    // echo "<!-- Debug: Query prepared -->"; flush();
    if(!$baglanti) { echo "Query Preparation Failed"; exit; } // Added from user's edit
    $baglanti->execute();
...
function sortLink($col, $label, $currentSort, $currentOrder) {
    ...
    $params = $_GET; // Note: In PHP < 8 this might be null if no GET, but usually it works.
    $params['sort'] = $col;
                ...
                <table id="shTable" class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th><?php echo sortLink('et.son_guncelleme', 'Son Güncelleme', $sort, $order); ?></th>
                    ...
                    <tbody>
                        ...
                        while ($listele = veriliste($baglanti)) {  
                            ...
                            // Date Format
                            $guncellemeTarihi = isset($listele['son_guncelleme']) ? strtotime($listele['son_guncelleme']) : null;
                            $guncellemeStr = $guncellemeTarihi ? date('d.m.Y H:i', $guncellemeTarihi) : '-';
                            if (!$guncellemeTarihi && isset($listele['kayit_tarihi'])) {
                                $guncellemeStr = date('d.m.Y H:i', strtotime($listele['kayit_tarihi']));
                            }
                        ?>
                        <tr>
                            ...
                            <td>
                                <span class="small text-muted"><i class="bi bi-clock me-1"></i><?php echo $guncellemeStr; ?></span>
                            </td>
                            <td>
                                <?php 
