...
export default async function BlogDetailPage({ params }: { params: { slug: string } }) {
  ...
  const title = post.translations?.tr?.title || post.slug;
  const content = post.translations?.tr?.content || '';

  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white py-24">
      <div className="container mx-auto px-4 max-w-3xl">
        <Link href="/blog" className="text-[#d4af37] hover:text-white transition-colors mb-8 inline-flex items-center gap-2 text-sm uppercase tracking-wider">
          &larr; Bloga Dön
        </Link>
